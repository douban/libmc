...
#define ASSERT_N_STREQ(S1, S2, N)                    \
  do {                                               \
    ASSERT_TRUE(0 == std::strncmp((S1), (S2), (N))); \
  } while (0)
...
#define _mc_output_stderr(LEVEL, FORMAT, ...)                              \
  (fprintf(stderr, "[" PROJECT_NAME "] [" #LEVEL "] [%s:%d] " FORMAT "\n", \
...
#define _ASSERTION_FAILED(cond)                                                \
  (_mc_output_stderr(PANIC, "failed assertion `%s'", #cond), printBacktrace(), \
   __VOID_CAST(0))
#else
...
#define ASSERT(cond) ((cond) ? __VOID_CAST(0) : _ASSERTION_FAILED(cond))

#define NOT_REACHED() ASSERT(0)

#if MC_LOG_LEVEL >= MC_LOG_LEVEL_INFO
#define log_info(M, ...) \
  (_mc_output_stderr(INFO, M, ##__VA_ARGS__), __VOID_CAST(0))
#else
