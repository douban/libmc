...
void* client_create() { return new Client(); }
...
const char* client_get_server_address_by_key(void* client, const char* key,
                                             size_t key_len) {
  douban::mc::Client* c = static_cast<Client*>(client);
  return c->getServerAddressByKey(key, key_len);
}

const char* client_get_realtime_server_address_by_key(void* client,
                                                      const char* key,
                                                      size_t key_len) {
  douban::mc::Client* c = static_cast<Client*>(client);
...
int client_version(void* client, broadcast_result_t** results,
                   size_t* n_hosts) {
  douban::mc::Client* c = static_cast<Client*>(client);
...
#define IMPL_RETRIEVAL_CMD(M)                                        \
  int client_##M(void* client, const char* const* keys,              \
                 const size_t* key_lens, size_t n_keys,              \
...
#define IMPL_STORAGE_CMD(M)                                                 \
  int client_##M(void* client, const char* const* keys,                     \
                 const size_t* key_lens, const flags_t* flags,              \
                 const exptime_t exptime, const cas_unique_t* cas_uniques,  \
                 const bool noreply, const char* const* vals,               \
                 const size_t* val_lens, size_t nItems,                     \
                 message_result_t*** results, size_t* n_results) {          \
    douban::mc::Client* c = static_cast<Client*>(client);                   \
    return c->M(keys, key_lens, flags, exptime, cas_uniques, noreply, vals, \
                val_lens, nItems, results, n_results);                      \
  }
...
int client_touch(void* client, const char* const* keys, const size_t* key_lens,
                 ...
                 message_result_t*** results, size_t* n_results) {
  douban::mc::Client* c = static_cast<Client*>(client);
  return c->touch(keys, key_lens, exptime, noreply, n_items, results,
                  n_results);
}
...
int client_stats(void* client, broadcast_result_t** results,
                 size_t* n_servers) {
  douban::mc::Client* c = static_cast<Client*>(client);
