...
void Client::config(config_options_t opt, int val) {
  switch (opt) {
    ...
    case CFG_HASH_FUNCTION:
      ConnectionPool::setHashFunction(
          static_cast<hash_function_options_t>(val));
    default:
      break;
  }
}

err_code_t Client::get(const char* const* keys, const size_t* keyLens,
                       size_t nKeys, retrieval_result_t*** results,
                       size_t* nResults) {
...
err_code_t Client::gets(const char* const* keys, const size_t* keyLens,
                        size_t nKeys, retrieval_result_t*** results,
                        size_t* nResults) {
...
void Client::collectRetrievalResult(retrieval_result_t*** results,
                                    size_t* nResults) {
...
void Client::collectMessageResult(message_result_t*** results,
                                  size_t* nResults) {
...
#define IMPL_STORAGE_CMD(M, O)                                                \
  err_code_t Client::M(                                                       \
      const char* const* keys, const size_t* keyLens, const flags_t* flags,   \
      const exptime_t exptime, const cas_unique_t* cas_uniques,               \
      const bool noreply, const char* const* vals, const size_t* valLens,     \
      size_t nItems, message_result_t*** results, size_t* nResults) {         \
    dispatchStorage((O), keys, keyLens, flags, exptime, cas_uniques, noreply, \
                    vals, valLens, nItems);                                   \
    err_code_t rv = waitPoll();                                               \
...
void Client::collectBroadcastResult(broadcast_result_t** results,
                                    size_t* nHosts) {
...
void Client::destroyBroadcastResult() {
  ConnectionPool::reset();
  for (std::vector<broadcast_result_t>::iterator it =
           m_outBroadcastResultPtrs.begin();
...
err_code_t Client::touch(const char* const* keys, const size_t* keyLens,
                         ...
                         size_t nItems, message_result_t*** results,
                         size_t* nResults) {
...
void Client::collectUnsignedResult(unsigned_result_t** results,
                                   size_t* nResults) {
...
err_code_t Client::incr(const char* key, const size_t keyLen,
...
err_code_t Client::decr(const char* key, const size_t keyLen,
                        const uint64_t delta, const bool noreply,
                        unsigned_result_t** results, size_t* nResults) {
...
void Client::_sleep(uint32_t seconds) { usleep(seconds * 1000000); }
