...
void PacketParser::setMode(ParserMode md) { m_mode = md; }
...
size_t PacketParser::requestKeyCount() { return m_requestKeys.size(); }

void PacketParser::process_packets(err_code_t& err) {
  // NOTE: always return with err RET_INCOMPLETE_BUFFER_ERR if not all packets
  // are recved.
  //
  // m_n_active--;
    ...
    switch (m_state) {
      ...
      case FSM_GET_FLAG:  // got "flag "
      {
        assert(mt_kvPtr != NULL &&
               mt_kvPtr->bytes + 1 >= mt_kvPtr->bytesRemain);
        uint64_t bytes;
        READ_UNSIGNED(bytes);
      ...
      case FSM_GET_VALUE_REMAINING:  // not got "\n" + all bytes + "\r\n"
      {
        ...
        if (mt_kvPtr->bytesRemain == mt_kvPtr->bytes) {
          ...
          if (m_buffer_reader->readLeft() < mt_kvPtr->bytes + 2) {
            m_buffer_reader->setNextPreferedDataBlockSize(
                mt_kvPtr->bytes + 2 - m_buffer_reader->readLeft());
          }
          m_buffer_reader->readBytes(err, mt_kvPtr->bytes,
                                     mt_kvPtr->data_block);
          if (err != RET_OK) {
            return;
          }
  ...
  switch (c1) {
    ...
    case 'E': {
      ...
      if (c2 == 'R') {
        ...
        log_err("error: [%.*s]", static_cast<int>(n - 1),
                ptr);  // -1 to ignore '\r'
    ...
    case 'S': {
      ...
      if (c2 == 'T') {
        ...
        assert(err == RET_OK);
        char* ptr = parseTokenData(err_td, n);
        log_err("server_error: [%.*s]", static_cast<int>(n - 1),
                ptr);  // -1 to ignore '/r'
        freeTokenData(err_td);
    ...
    case 'C': {
      ...
      assert(err == RET_OK);
      char* ptr = parseTokenData(err_td, n);
      log_err("client_error: [%.*s]", static_cast<int>(n - 1),
              ptr);  // -1 to ignore '/r'
      freeTokenData(err_td);
      err = RET_PROGRAMMING_ERR;
      m_state = FSM_ERROR;
...
types::LineResultList* PacketParser::getLineResults() { return &m_lineResults; }
