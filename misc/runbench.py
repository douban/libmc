...
class Prefix(object):
    ...
    def __getattr__(self, name):
        if name in ('get', 'add', 'replace', 'set', 'cas', 'delete', 'incr',
                    'decr', 'prepend', 'append', 'touch', 'expire', 'gets'):
            def func(key, *args, **kwargs):
                key = self.__prepare_prefix_keys(key)
                return getattr(self.mc, name)(key, *args, **kwargs)
            return func
        elif name in ('get_multi', 'append_multi', 'prepend_multi',
                      'delete_multi', 'set_multi', 'get_list'):
...
participants = [
    ...
    Participant(
        ...
        factory=lambda: Prefix(__import__('memcache').Client(servers),
                               'memcache1')
    ),

    Participant(
        name='libmc(md5 / ketama / nodelay / nonblocking, from douban)',
        factory=lambda: Prefix(__import__('libmc').Client(
